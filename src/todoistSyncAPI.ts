...
export class TodoistSyncAPI {
	...
	async getAllResources() {
		...
		try {
			...
			if (!response.ok) {
				throw new Error(
					`Failed to fetch all resources: ${response.status} ${response.statusText}`
				);
			}
			...
			console.error(error);
			throw new Error(
				"Failed to fetch all resources due to network error"
			);
		}
	}
	...
	async getUserResource() {
		...
		try {
			...
			console.error(error);
			throw new Error(
				"Failed to fetch user resources due to network error"
			);
		}
	}
	...
	async updateUserTimezone() {
		const unixTimestampString: string = Math.floor(
			Date.now() / 1000
		).toString();
		const accessToken = this.plugin.settings.todoistAPIToken;
		const url = "https://api.todoist.com/sync/v9/sync";
	...
	async getCompletedItemsActivity() {
		...
		try {
			...
			if (!response.ok) {
				throw new Error(
					`Failed to fetch completed items: ${response.status} ${response.statusText}`
				);
			}
			...
			console.error(error);
			throw new Error(
				"Failed to fetch completed items due to network error"
			);
		}
	}
	...
	async getUncompletedItemsActivity() {
		...
		try {
			...
			if (!response.ok) {
				throw new Error(
					`Failed to fetch uncompleted items: ${response.status} ${response.statusText}`
				);
			}
			...
			console.error(error);
			throw new Error(
				"Failed to fetch uncompleted items due to network error"
			);
		}
	}
	...
	async getNonObsidianCompletedItemsActivity() {
		...
		//client中不包含obsidian 的activity
		const filteredArray = completedItemsActivityEvents.filter(
			(obj) => !obj.extra_data.client.includes("obsidian")
		);
		return filteredArray;
	}
	...
	async getNonObsidianUncompletedItemsActivity() {
		const uncompletedItemsActivity =
			await this.getUncompletedItemsActivity();
		const uncompletedItemsActivityEvents = uncompletedItemsActivity.events;
		//client中不包含obsidian 的activity
		const filteredArray = uncompletedItemsActivityEvents.filter(
			(obj) => !obj.extra_data.client.includes("obsidian")
		);
		return filteredArray;
	}
	...
	async getUpdatedItemsActivity() {
		...
		try {
			...
			if (!response.ok) {
				throw new Error(
					`Failed to fetch updated items: ${response.status} ${response.statusText}`
				);
			}
			...
			console.error(error);
			throw new Error(
				"Failed to fetch updated items due to network error"
			);
		}
	}
	...
	async getProjectsActivity() {
		...
		try {
			...
			if (!response.ok) {
				throw new Error(
					`Failed to fetch  projects activities: ${response.status} ${response.statusText}`
				);
			}
			...
			console.error(error);
			throw new Error(
				"Failed to fetch projects activities due to network error"
			);
		}
	}
}
